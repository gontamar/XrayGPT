...
def gradio_reset(chat_state, img_list):
    ...
    if img_list is not None:
        img_list.clear()
    return None, gr.update(value=None, interactive=True), gr.update(placeholder='Please upload your image first', interactive=False), gr.update(value="Upload and Ask Queries", interactive=True), chat_state, img_list
...
def upload_img(gr_img, text_input, chat_state):
    ...
    # Enable text input and disable upload button (to prevent double upload)
    return gr.update(value=gr_img, interactive=False), gr.update(interactive=True, placeholder='Type your question here'), gr.update(value="Start Chatting", interactive=False), chat_state, img_list
...
def set_example_xray(example):
    # example is a list of 1 element [image_path]
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            image = gr.Image(type="pil", label="Upload X-ray image")
        ...
        with gr.Column():
            ...
            text_input = gr.Textbox(label='Your Query', placeholder='Please upload your X-Ray image first.', interactive=False)
    ...
    # We use gr.Examples instead of deprecated gr.Dataset
    gr.Examples(
        examples=[[path] for path in example_xray_paths],
        inputs=image,
    ...
    # When user clicks example prompt, load text and then start chat
    def on_prompt_example_selected(example_text):
        # returns new text_input and clears chat and keeps chat state and img_list
        return example_text[0], [], CONV_VISION.copy()
    ...
    gr.Examples(
        ...
        label="Prompt Examples",
        cache_examples=False,
    ).style(container=False).click(
        fn=set_example_text_input, inputs=text_input, outputs=text_input
    ).then(
